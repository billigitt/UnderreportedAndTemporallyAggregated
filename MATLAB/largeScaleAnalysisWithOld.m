...
load('../MATs/largeScaleStudyOriginalMethodClusterSeed1.mat')
...
largeScaleCrIWidthOld = largeScaleOld.upperRt - largeScaleOld.lowerRt;
idxUninformativeOld = largeScaleCrIWidthOld > cri95WidthThreshold;
idxInformativeOld = largeScaleCrIWidthOld < cri95WidthThreshold;
largeScaleCrIWidthOld(idxUninformativeOld) = nan;

logicalTotalCoverageOld = ((largeScaleOld.trueR >= largeScaleOld.lowerRt) & (largeScaleOld.trueR <= largeScaleOld.upperRt));
overallTotalCoverageOld = sum(logicalTotalCoverageOld(idxInformativeOld))/length(logicalTotalCoverageOld(idxInformativeOld));
logicalTotalCoverageOld(idxUninformativeOld) = false;
logicalCoverageBySimOld = reshape(logicalTotalCoverageOld, [10, 900]);
consideredInferencesBySimOld = reshape(idxInformativeOld, [10, 900]);
coverageBySimOld = sum(logicalCoverageBySimOld, 1)./sum(consideredInferencesBySimOld, 1);

absoluteRelativeErrorOld = abs(largeScaleOld.trueR - largeScaleOld.meanRt)./largeScaleOld.trueR;

mean(absoluteRelativeErrorOld)
...
absoluteRelativeErrorNaiveEE = abs(largeScaleStudyFinal.trueR(idxInformativeEE) - meansEE(idxInformativeEE))./largeScaleStudyFinal.trueR(idxInformativeEE);
...
k = 1;
% NB: WE HAVE CHANGED LENGTH OF ABSOLUTERELATIEVERRORS BECAUSE WE NOW TAKE
% OUT IDXUNINFORMATIVE
