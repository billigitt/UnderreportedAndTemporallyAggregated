...
incidenceAndTrueR = CSV.read("../CSVs/largeScaleStudyIncidencesAndTrueRsNoLimitPrior1And3FirstDay1TemporalRho.csv", DataFrame)
Random.seed!(1)
...
maxEntireIter = defaultM^2
criCheck = true

for i in 1:nEpidemics

    x = inferTempAggOnlyR(incidenceAndTrueR.reportedWeeklyI[((i-1)*T+1):(i*T)], wAssumed, priorRShapeAndScale, defaultM, defaultP, maxIter, maxEntireIter)
